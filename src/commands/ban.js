...
module.exports = {
    ...
    execute(client, message, args, currency, category, distube, tmpMsg) {
        ...
        if (!message.member.permissions.has(Permissions.FLAGS.BAN_MEMBERS, Permissions.FLAGS.ADMINISTRATOR) && message.member != message.guild.ownerId) return message.channel.send(":x: You do not have permissions to ban members!\nRequires `BAN\_MEMBERS` or `ADMINISTRATOR`");
